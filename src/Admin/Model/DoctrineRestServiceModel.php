...
class DoctrineRestServiceModel implements EventManagerAwareInterface
{
    ...
    public static function onFetch($event)
    {
        ...
        $configResource = $config['api-tools']['doctrine-connected'][$entity->resourceClass];
    ...
    public function updateDoctrineConfig(DoctrineRestServiceEntity $original, DoctrineRestServiceEntity $update): void
    {
        ...
        $basekey                 = 'api-tools.doctrine-connected.';
    ...
    public function deleteFiles(DoctrineRestServiceEntity $entity): void
    {
        ...
        $restResourceClass   = $config['api-tools-rest'][$entity->controllerServiceName]['listener'];
        $restCollectionClass = $config['api-tools-rest'][$entity->controllerServiceName]['collection_class'];
    ...
    public function deleteRoute(DoctrineRestServiceEntity $entity): void
    {
        ...
        $uriKey = array_search($route, $config['api-tools-versioning']['uri']);
        if ($uriKey !== false) {
            $key = ['api-tools-versioning', 'uri', $uriKey];
            $this->configResource->deleteKey($key);
        }
    }
    ...
    public function deleteDoctrineRestConfig(DoctrineRestServiceEntity $entity): void
    {
        ...
        $key = ['api-tools', 'doctrine-connected', $entity->resourceClass];
        $this->configResource->deleteKey($key);

        $key = ['api-tools-rest', $entity->controllerServiceName];
        $this->configResource->deleteKey($key);

        $key = ['api-tools-content-negotiation', 'controllers', $entity->controllerServiceName];
        $this->configResource->deleteKey($key);

        $key = ['api-tools-content-negotiation', 'accept_whitelist', $entity->controllerServiceName];
        $this->configResource->deleteKey($key);

        $key = ['api-tools-content-negotiation', 'content_type_whitelist', $entity->controllerServiceName];
        $this->configResource->deleteKey($key);

        $key = ['api-tools-hal', 'metadata_map', $entity->collectionClass];
        $this->configResource->deleteKey($key);

        $key = ['api-tools-hal', 'metadata_map', $entity->entityClass];
        $this->configResource->deleteKey($key);

        $validator = $config['api-tools-content-validation'][$entity->controllerServiceName]['input_filter'];

        $key = ['api-tools-content-validation', $entity->controllerServiceName];
        $this->configResource->deleteKey($key);
